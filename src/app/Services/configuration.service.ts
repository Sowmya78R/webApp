...
export class ConfigurationService {
  ...
  constructor(private httpService: HttpService, private appService: AppService
    ...
    if (this.userDetails && this.userDetails.wareHouseConfiguration && this.userDetails.wareHouseConfiguration.wareHouseFunctionalities.length > 0) {
      this.organizationDropdown = this.userDetails.wareHouseConfiguration.wareHouseFunctionalities.map((x: { organizationInfo: { organizationIDName: any; }; }) => x.organizationInfo.organizationIDName);
      this.wareHouseDropdown = this.userDetails.wareHouseConfiguration.wareHouseFunctionalities.map((x: { wareHouseInfo: { wareHouseIDName: any; }; }) => x.wareHouseInfo.wareHouseIDName);
  ...
  getThemeContent(value: any) {
    if (this.userDetails && this.userDetails.themeConfigurations && this.userDetails.themeConfigurations.length > 0) {
      const themeConfig = this.userDetails.themeConfigurations.find((x: { organizationInfo: { organizationIDName: any; }; wareHouseInfo: { wareHouseIDName: null; }; }) => x.organizationInfo.organizationIDName == this.getGlobalpayload().organizationIDName && x.wareHouseInfo.wareHouseIDName == this.getGlobalpayload().wareHouseIDName);
      if (themeConfig) {
        const filteredOne = themeConfig.themeProcesses.find((x: { name: any; }) => x.name == value);
  ...
  getOrganization() {
    ...
    if (this.organizationValue && this.userDetails && this.userDetails.wareHouseConfiguration && this.userDetails.wareHouseConfiguration.wareHouseFunctionalities && this.userDetails.wareHouseConfiguration.wareHouseFunctionalities.length > 0) {
      org = this.userDetails.wareHouseConfiguration.wareHouseFunctionalities.find((x: { organizationInfo: { organizationIDName: any; }; }) => x.organizationInfo.organizationIDName == this.organizationValue).organizationInfo;
    }
    return org;
  }
  getWarehouse() {
    ...
    if ((this.wareHouseValue != "null") && this.userDetails && this.userDetails.wareHouseConfiguration && this.userDetails.wareHouseConfiguration.wareHouseFunctionalities && this.userDetails.wareHouseConfiguration.wareHouseFunctionalities.length > 0) {
      org = this.userDetails.wareHouseConfiguration.wareHouseFunctionalities.find((x: { wareHouseInfo: { wareHouseIDName: any; }; }) => x.wareHouseInfo.wareHouseIDName == this.wareHouseValue).wareHouseInfo;
      // Constants.WAREHOUSE_ID = org['wareHouseMasterID']
    }
    return org;
  }
  setWareHouseRelatedorg(warehouseIDName: string, key?: string) {
    ...
    if (warehouseIDName != 'null') {
      const index = this.userDetails.wareHouseConfiguration.wareHouseFunctionalities
        .findIndex((x: { wareHouseInfo: { wareHouseIDName: string; }; }) => x.wareHouseInfo.wareHouseIDName == warehouseIDName);
      if (index != -1) {
