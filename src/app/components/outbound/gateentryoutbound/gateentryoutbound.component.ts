...
import { gateEntryForOutBoundHead } from 'src/app/constants/paginationConstants';

@Component({
  selector: 'app-gateentryoutbound',
  templateUrl: './gateentryoutbound.component.html',
  styleUrls: ['./gateentryoutbound.component.scss']
})
export class GateentryoutboundComponent implements OnInit {
  ...
  getAllFunctions() {
    this.permissionsList.includes('View')
    {
      ...
      this.findAllUnits();
      this.fetchAllExecutionIDName('Loading');
  ...
  onWMSONoChange(event) {
    if (event) {
      ...
      if (filteredOne) {
        ...
        this.GRNoteForm.controls.waybillNumber.patchValue(filteredOne.wayBillNumber);
        // modeofTransport
      }
    }
  }
  ...
  statusOptions = [
    { label: 'Sales', value: 'Sales', color: 'grey' },
    { label: 'Quality Check', value: 'Quality Check', color: 'ourCommonColor' },
    { label: 'Loading', value: 'Loading', color: 'grey' },
    { label: 'Shipment', value: 'Shipment', color: 'darkcyan' }
  ];
  ...
  fetchAllDFirst(first, event) {
    if (first) {
      this.getAll(first, event.target.value);
    }
  }
  page: number = 1;
  itemsPerPage = 10;
  ...
  setDirection(type, headerName) {
    ...
    let arr: any = gateEntryForOutBoundHead['gateEntrySortFieldsArrays'].find(x => x.key == headerName);
  ...
  tableHeadings: any = ['S.No', 'Invoice Number', 'Gate In D&T', 'Gate Out D&T', 'Order ID', 'Supplier/Customer/Warehouse IDName',
    'Invoice Date', 'Vehicle Number', 'Vehicle Type', 'Transporter', 'Way Bill Number', 'Invoice Qty', 'Assigned To', 'Assigned By', 'Start Time',
    ...
    , 'Planned Completion Date', 'Loading Status', 'Assigned To', 'Assigned By', 'Start Time', 'End Time', 'Completed By', 'Planned Completion Date', 'Shipment Status'
    , 'Status', 'Action'];
