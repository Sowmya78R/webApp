...
@Component({
  selector: 'app-ontimeordervolumedeliverydashboard',
  templateUrl: './ontimeordervolumedeliverydashboard.component.html',
  styleUrls: ['./ontimeordervolumedeliverydashboard.component.scss']
})
export class OntimeordervolumedeliverydashboardComponent implements OnInit {
  ...
  onTimeDeliveryQuantityForm: FormGroup
  ...
  getGraphData(payload) {
    if (payload.type) {
      ...
      this.dashboardService.fetchonTimeDeliveryVolVsDate(payload).subscribe(res => {
  ...
  createonTimeDeliveryQuantityForm() {
    this.onTimeDeliveryQuantityForm = this.fb.group({
