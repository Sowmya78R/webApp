...
export class ReportConfigComponent implements OnInit {
  ...
  ngOnInit() {
    this.configPermissionsList = this.configurationService.getConfigurationPermissions('mainFunctionalities', 'Reports Config', this.getClientRole());
  ...
  createProcessGroupsForm() {
    this.reportForm = new FormBuilder().group({
      role: [null],
  ...
  onSelectUser(event) {
    ...
    if (event) {
      if (this.yesAdmin) {
        ...
        this.reportForm.controls.userInfo.patchValue(userDet);
        this.fetchTemplate();
      }
      else {
        this.commonMasterDataService.findOrgandWarehouseByUserIdName(event.originalObject).subscribe(res => {
          if (res['status'] == 0 && res['data']['organizationWareHouseHelpers']) {
            ...
            this.reportForm.controls.userInfo.patchValue(this.users.find(x => x.userIDName == event.originalObject))
  ...
  onSelectWareHouse(event) {
    if (event) {
      this.getPermissions(this.reportForm.value.userInfo.userIDName, this.childOrganizationDropdown, event.originalObject);
    }
    else {
      this.resetIsEnabledFlag();
      this.permissionsList = [];
    ...
    this.configurationService.getFunctionalitiesbyUserIDName(user).subscribe(
      (response) => {
        if (response && response.status === 0 && response.data.userPermissionFunctionality && response.data.userPermissionFunctionality.userPermissionWareHouseFunctionality.length > 0) {
          ...
          this.roleSpecificFunctionalities = individualFunctionality ? individualFunctionality.reportsFunctionalities : [];
          ...
          this.completeResponse = {
            userInfo: this.reportForm.controls.userInfo.value,
  ...
  fetchTemplate() {
    this.configurationService.fetchDBWisePermissionsTemplate({ "email": this.selectedUser }).subscribe(res => {
      if (res.status == 0 && res['data'].functionality) {
        ...
        if (this.yesAdmin) {
          if (this.clientTemplate.reportsFunctionalities && this.clientTemplate.reportsFunctionalities.length > 0) {
            this.functionalities.forEach((functionality, i) => {
              functionality.subFunctionalities.forEach((subFunctionality, j) => {
                this.clientTemplate.reportsFunctionalities.forEach((rFunctionality, k) => {
                  rFunctionality.subFunctionalities.forEach((rSubFunctionality, l) => {
                    if (subFunctionality.name === rSubFunctionality.name) {
                      this.functionalities[i].subFunctionalities[j].isEnabled = true;
                      this.functionalities[i].subFunctionalities[j]['Update'] = (rSubFunctionality.permissions && rSubFunctionality.permissions.length > 0 && this.permissionsList.includes('Update')) ? (rSubFunctionality.permissions.find(x => x.permissionName == 'Update') ? true : false) : false;
          ...
          this.functionalities = this.clientTemplate.reportsFunctionalities ? this.clientTemplate.reportsFunctionalities : this.functionalities;
          this.resetIsEnabledFlag();
        }
      }
      else {
  ...
  fetchAllFunctionalities() {
    this.configurationService.findAllFunctionalitiesByRoleID(this.role).subscribe(
      (response) => {
        if (response && response.status === 0 && response.data.functionality && response.data.functionality.reportsFunctionalities) {
          if (this.role === 'ROLE_TEMPLATE') {
            this.completeResponse = response.data.functionality;
            this.functionalities = response.data.functionality.reportsFunctionalities;
            this.resetIsEnabledFlag();
          } else {
            this.resetIsEnabledFlag();
            this.completeResponse = response.data.functionality;
            this.roleSpecificFunctionalities = response.data.functionality.reportsFunctionalities;
            this.functionalities.forEach((functionality, i) => {
              functionality.subFunctionalities.forEach((subFunctionality, j) => {
                this.roleSpecificFunctionalities.forEach((rFunctionality, k) => {
                  rFunctionality.subFunctionalities.forEach((rSubFunctionality, l) => {
  ...
  update() {
    if (this.configPermissionsList.includes('Update')) {
      if ((this.yesAdmin && this.reportForm.controls.userInfo.value.userIDName) || this.reportForm.controls.userInfo.value && this.childOrganizationDropdown && this.childWareHouseDropdown) {
        ...
        if (this.yesAdmin) {
          if (this.clientTemplate && this.clientTemplate._id) {
            this.clientTemplate.reportsFunctionalities = endData;
          }
          else {
            this.clientTemplate = {
              "userInfo": this.reportForm.controls.userInfo.value,
              "_id": null,
              "name": "WMS Functionality Names",
              "reportsFunctionalities": endData
            }
          }
          ...
          this.completeResponse.userPermissionWareHouseFunctionality[index].reportsFunctionalities = endData;
