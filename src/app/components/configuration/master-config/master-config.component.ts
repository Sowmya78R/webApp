...
export class MasterConfigComponent implements OnInit {
  ...
  selectAllDeleteCheckBox: boolean = false;
  selectAllCreateCheckBox: boolean = false;
  ...
  ngOnInit() {
    this.configPermissionsList = this.configurationService.getConfigurationPermissions('mainFunctionalitiesForMasters', 'Masters Config', this.getClientRole());
      ...
      this.functionalities.forEach((functionality, i) => {
        functionality.subFunctionalities.forEach((subFunctionality, j) => {
          if (key == 'isEnabled') {
            ...
            (key == 'Update') ? (subFunctionality.Update = false) : ((key == 'Delete')
             ? subFunctionality.Delete = false : (key == 'Create') ? (subFunctionality.Create = false) : '');
          }
        })
      })
    }
  ...
  isAllSelected(key) {
    ...
    if (key == 'isEnabled') {
      ...
      this.selectAllCreateCheckBox = formArray.every(function (item: any) {
      ...
      this.selectAllDeleteCheckBox = formArray.every(function (item: any) {
      ...
      (response) => {
        if (response && response.status === 0 && response.data.users) {
          ...
          if (!this.yesAdmin) {
            ...
            this.selectedUser = this.users.find(x => x.businessUnit == 
              businessUnit && x.rolesList[0].roleName == 'ROLE_CLIENT').email;
            this.fetchTemplate();
          }
        } else {
          this.users = [];
  ...
  onSelectUser(event) {
    ...
    this.selectAllCreateCheckBox = false;
    this.selectAllDeleteCheckBox = false;
    this.orgData = [];
    this.warehouses = [];
    ...
    if (event) {
      if (this.yesAdmin) {
        ...
        this.masterForm.controls.userInfo.patchValue(userDet);
        this.fetchTemplate();
      }
      else {
        this.commonMasterDataService.findOrgandWarehouseByUserIdName(event.originalObject).subscribe(res => {
          if (res['status'] == 0 && res['data']['organizationWareHouseHelpers']) {
            ...
            this.masterForm.controls.userInfo.patchValue(this.users.find(x => x.userIDName == event.originalObject))
  ...
  onSelectWareHouse(event) {
    if (event) {
      this.getPermissions(this.masterForm.value.userInfo.userIDName, this.childOrganizationDropdown, event.originalObject);
    }
    else {
      this.resetIsEnabledFlag();
      this.permissionsList = [];
  ...
  createProcessGroupsForm() {
    this.masterForm = new FormBuilder().group({
      role: [null],
  ...
  fetchTemplate() {
    this.configurationService.fetchDBWisePermissionsTemplate({ "email": this.selectedUser }).subscribe(res => {
      if (res.status == 0 && res['data'].functionality) {
        ...
        if (this.yesAdmin) {
          if (this.clientTemplate.mainFunctionalitiesForMasters && this.clientTemplate.mainFunctionalitiesForMasters.length > 0) {
            this.functionalities.forEach((functionality, i) => {
              functionality.subFunctionalities.forEach((subFunctionality, j) => {
                this.clientTemplate.mainFunctionalitiesForMasters.forEach((rFunctionality, k) => {
                  rFunctionality.subFunctionalities.forEach((rSubFunctionality, l) => {
                    if (subFunctionality.name === rSubFunctionality.name) {
                      ...
                      this.functionalities[i].subFunctionalities[j]['Create'] = (rSubFunctionality.permissions && rSubFunctionality.permissions.length > 0) ? (rSubFunctionality.permissions.find(x => x.permissionName == 'Create') ? true : false) : false;
                    }
                  });
                });
              });
          ...
          this.functionalities = this.clientTemplate.mainFunctionalitiesForMasters ? this.clientTemplate.mainFunctionalitiesForMasters : this.functionalities;
          this.resetIsEnabledFlag();
        }
      }
      else {
  ...
  fetchAllFunctionalities() {
    this.configurationService.findAllFunctionalitiesByRoleID(this.role).subscribe(
      (response) => {
        if (response && response.status === 0 && response.data.functionality && response.data.functionality.mainFunctionalitiesForMasters) {
          if (this.role === 'ROLE_TEMPLATE') {
            this.completeResponse = response.data.functionality;
            this.functionalities = response.data.functionality.mainFunctionalitiesForMasters;
            this.resetIsEnabledFlag();
          }
        } else if (response && response.status === 0 && response.data.functionality && response.data.functionality.mainFunctionalitiesForMasters) {
          this.resetIsEnabledFlag();
          this.completeResponse = response.data.functionality;
        } else {
          this.resetIsEnabledFlag();
    ...
    this.configurationService.getFunctionalitiesbyUserIDName(user).subscribe(
      (response) => {
        if (response && response.status === 0 && response.data.userPermissionFunctionality && response.data.userPermissionFunctionality.userPermissionWareHouseFunctionality.length > 0) {
          ...
          this.roleSpecificFunctionalities = individualFunctionality ? individualFunctionality.mainFunctionalitiesForMasters : [];
          ...
          this.completeResponse = {
            userInfo: this.masterForm.controls.userInfo.value,
  ...
  getCheckedReport(reportValue, isChecked) {
      ...
      process.subFunctionalities.forEach((subProcess, j) => {
        if (subProcess.name === reportValue) {
          ...
          if (!isChecked.target.checked) {
            this.functionalities[i].subFunctionalities[j]['Create'] = isChecked.target.checked;
            this.functionalities[i].subFunctionalities[j]['Update'] = isChecked.target.checked;
            this.functionalities[i].subFunctionalities[j]['Delete'] = isChecked.target.checked;
            this.isAllSelected('Create');
            this.isAllSelected('Update');
            this.isAllSelected('Delete');
          }
  ...
  update() {
    if (this.configPermissionsList.includes('Update')) {
      if ((this.yesAdmin && this.masterForm.controls.userInfo.value.userIDName) || this.masterForm.controls.userInfo.value && this.childOrganizationDropdown && this.childWareHouseDropdown) {
        ...
        reqJSON.forEach((functionality, i) => {
          for (const subFunctionality of functionality.subFunctionalities) {
            ...
            if (subFunctionality.isEnabled === true) {
              ...
              if (subFunctionality.Create) {
                funcPermissions.push({ permissionName: 'Create' })
              }
              if (subFunctionality.Delete) {
        ...
        if (this.yesAdmin) {
          if (this.clientTemplate && this.clientTemplate._id) {
            this.clientTemplate.mainFunctionalitiesForMasters = endData;
          }
          else {
            this.clientTemplate = {
              "userInfo": this.masterForm.controls.userInfo.value,
              "_id": null,
              "name": "WMS Functionality Names",
              "mainFunctionalitiesForMasters": endData
            }
          }
          ...
          this.completeResponse.userPermissionWareHouseFunctionality[index].mainFunctionalitiesForMasters = endData;
