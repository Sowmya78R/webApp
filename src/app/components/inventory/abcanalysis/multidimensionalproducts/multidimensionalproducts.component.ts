...
@Component({
  selector: 'app-multidimensionalproducts',
  templateUrl: './multidimensionalproducts.component.html',
  styleUrls: ['./multidimensionalproducts.component.scss']
})
export class MultidimensionalproductsComponent implements OnInit {
  arr: any = [];
  headersArrayDate: any = [];
  productCoefficientInfoArr: any = [];
  overAllDates: any = [];
  entryFieldsArrayData: any = [];
  multiDimentionalproductForm: FormGroup
  ...
  constructor(private wmsService:WMSService,
   ...
   this.wmsService.abcDataSharing.subscribe((data) => {
     this.createMultiDimentionalProductForm();

     this.valueimg = data;
    console.log(data);
     if (data) {
       const customerReq = this.multiDimentionalproductForm.value;
     //this.multiDimentionalAnalysisForm.controls.type.setValue(data.type);
      this.multiDimentionalproductForm.controls.fromDate.setValue(data.fromDate);
       this.multiDimentionalproductForm.controls.toDate.setValue(data.toDate);
      /// console.log(this.regionWiseDetailsForm.value.type);
      console.log(this.multiDimentionalproductForm.value.fromDate);
      console.log(this.multiDimentionalproductForm.value.toDate);
       this.generate();
     }
   })
 }
  ...
  createMultiDimentionalProductForm() {
    this.multiDimentionalproductForm = this.fb.group({
      fromDate: [''],
      toDate: ['']
    })
  }
  ...
  finalArr: any = []
  generate() {
    const abcFormRequest = this.multiDimentionalproductForm.value;
  ...
  clearFields() {
    this.multiDimentionalproductForm.reset();
  }
