...
@Component({
  selector: 'app-billing-po',
  templateUrl: './billing-po.component.html',
  styles: [`
    fieldSet {
...
export class BillingPoComponent implements OnInit, OnDestroy {
  ...
  makeThisDisabledforSupplier: boolean
  ngOnDestroy(): void {
  ...
  disableFields() {
    if (this.permissionsList.includes('View') && this.permissionsList.includes('Update')) {
      ...
      this.makeThisDisabledforSupplier = false
      ...
      this.vasRestService.saveOrUpdateBillingPO(JSON.stringify(req)).subscribe(
        (response) => {
          if (response && response.status === 0 && response.data.billingPurchaseOrder) {
            this.billingPoID = response.data.billingPurchaseOrder;
            // this.clear();

            if (this.appService.getParam('id')) {
              this.toastr.success(`${this.billingPoID.billingpoNumber} - saved Billing PO successfully`);
            } else {
              this.toastr.success(`${this.billingPoID.billingpoNumber} - Created Billing PO successfully`);
            }
  ...
  updateRaisePO() {
    if (this.permissionsList.includes('Update')) {
      if (this.billingPoID && this.billingPoID._id) {
        this.vasRestService.updateRaisePO(this.billingPoID._id, this.formObj).subscribe(res => {
          if (res && res.status === 0 && res.data['BillingPurchaseOrder details ']) {
            this.toastr.success(`${this.billingPoID.billingpoNumber} - Raised PO Successfully`);
            this.clear();
            this.appService.navigate('/v1/vas/billing-po', null);
          } else {
  ...
  populateAmountLabourCharges(index) {
    ...
    if (data.ratePerHourStaff && data.noOfHoursOccupiedStaff) {
      ...
      this.labourChargesForm.controls.labourChargesAmount.setValue(totalAmount);
      this.labourChargesTotal = totalAmount;
      this.populateTotalAmount();
    }
  }
    ...
    this.commonMasterDataService.fetchAllVehicles(this.formObj).subscribe(
      (response) => {
        if (response && response.status === 0 && response.data.vehicleMasters) {
          ...
          //changed typeVehicle to vehicleType, becz data not getting in typeVehicle field.
      ...
      (response) => {
        if (response && response.status === 0 && response.data.equipmentMaster) {
          ...
          response.data.equipmentMaster.forEach(equipment => {
            this.equipmentTypes.push(equipment.equipmentType);
            //changed typeOfEquipment to equipmentType
          });
        }
      },
      (error) => {
